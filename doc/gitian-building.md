...
*Setup instructions for a gitian build of Tedcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tedcoin
...
- [Building Tedcoin](#building-tedcoin)
...
Clone the git repositories for Tedcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tedchain/tedcoin-immutable-voting-system
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tedcoin repository
...
To build Tedcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tedcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tedcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tedchain/tedcoin-immutable-voting-system
...
For example:
```bash
URL=https://github.com/tedchain/tedcoin-immutable-voting-system.git
COMMIT=master
./bin/gbuild --commit tedcoin=${COMMIT} --url tedcoin=${URL} ../tedcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tedcoin=${COMMIT} --url tedcoin=${URL} ../tedcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tedcoin=${COMMIT} --url tedcoin=${URL} ../tedcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tedcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tedcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tedcoin-osx-build.assert
```
