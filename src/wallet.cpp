    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s HYP %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, int nSplitBlock, bool fAllowS4C, const CCoinControl* coinControl)
{
    int64 nValue = 0;
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
						...
						// change Tedcoin to get rid of automatic generation of change address and instead send change back
...
{
    //presstab Tedcoin: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    // presstab Tedcoin - Initialize as static and don't update the set on every run of CreateCoinStake() in order to lighten resource use
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
    {
        ...
        if (foundKernel)
		{
			...
			//presstab Tedcoin - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
			uint64 nCoinAge;
			CTxDB txdb("r");
			const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBest, false);
			...
			//presstab Tedcoin - if MultiSend is set to send in coinstake we will add our outputs here (values asigned further down)
	...
	if(fCombineDust) //presstab Tedcoin - this combination code iterates through all of your outputs on successful coinstake, so its useful to have user be able to choose whether this is necessary
	{
		BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
		{
...
// Tedcoin: complete construction of a proposal transaction
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee, bool fAllowStakeForCharity)
{
    // Check amount
    if (nValue <= 0)
...
{
    {
        ...
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            ...
            printf("    stake: %d  %d  %lld", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
         }
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            printf("keypool added key %lld, size=%lu\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
