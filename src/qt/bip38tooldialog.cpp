...
{
    QApplication::clipboard()->setText(ui->encryptedKeyOut_ENC->text());
}
...
void Bip38ToolDialog::on_decryptKeyButton_DEC_clicked()
{
    ...
    if (!key.SetPrivKey_Raw(privKey, fCompressed)) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_DEC->setText(tr("Failed to set private key."));
        return;
    }
